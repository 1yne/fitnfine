...
async function saveUserDataToStore({ event, resolve }) {
...
async function loadUserWorkouts({ event, resolve }) {
  ...
  // let choices: any[] = responseData.choices[0].text.split(/\r?\n/).filter((val) => {
  //   return val.length !== 0;
  // });
  ...
  //     choices[i] = {
  //       ...randomWorkout,
  //       id: randomWorkout.name.replaceAll(" ", "_").toLocaleLowerCase()
  //     }
  //   }
  ...
  // if (choices.length > 0) {
  //   event.locals.workoutData = choices
  // }
